...
import { encodeFunctionData, parseEther, type Log, decodeEventLog } from "viem";
...
function EscrowItem({ event }: { event: EscrowEventInfo }) {
  ...
  return (
    ...
    >
      <p style={{ margin: 0 }}>
        Escrow {truncateHexString({ address: escrowAddress, length: 12 })}{" "}
        created (tx:{" "}
        <a
      ...
      {escrowInfo && (
        <ul
          style={{ listStyleType: "square", paddingLeft: "1.2rem", margin: 0 }}
        >
          <li>Payee: {payee}</li>
  ...
  return (
    <div>
      ...
      >
        ...
        <div>Latest Permissions keys: {latestPermissions?.key.publicKey}</div>
      ...
      {id && (
        <details style={{ marginTop: "5px" }}>
          <summary>
            ...
            {truncateHexString({
              address: typeof id === "string" ? id : (id as { id: string }).id,
              length: 12,
            })}
          </summary>
  ...
  useEffect(() => {
    ...
    for (const receipt of receipts) {
      for (const log of receipt.logs ?? []) {
        if (log.address.toLowerCase() !== EscrowFactory.address.toLowerCase())
          continue;
    ...
    <div>
      ...
      <form
        onSubmit={(event) => {
          event.preventDefault();
          // eslint-disable-next-line no-console
          console.info("[CreateEscrow] Submitting sendCalls", {
            data: calldata,
          });
          sendCalls({
            calls: [
