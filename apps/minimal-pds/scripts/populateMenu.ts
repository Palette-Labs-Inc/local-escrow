...
function putRecord<T extends { [_ in string]: unknown }>(
  collection: string,
  rkey: string,
  record: T
) {
...
async function populateCatalog() {
  const item1 = await putRecord<XyzNoshdeliveryV0CatalogItem.Record>(
  ...
  const item2 = await putRecord<XyzNoshdeliveryV0CatalogItem.Record>(
    "xyz.noshdelivery.v0.catalog.item",
    itemKeys[1],
    {
      name: "Salad",
  ...
  const item3 = await putRecord<XyzNoshdeliveryV0CatalogItem.Record>(
    "xyz.noshdelivery.v0.catalog.item",
    itemKeys[2],
    {
      name: "Burger",
  ...
  const collection1 =
    await putRecord<XyzNoshdeliveryV0CatalogCollection.Record>(
  ...
  const catalog = await putRecord<XyzNoshdeliveryV0CatalogCatalog.Record>(
    ...
    {
      merchantLocation:
        "at://" + REPO + "/xyz.noshdelivery.v0.merchant.location/3lnw7k6v34w25",
      name: "Full Menu",
